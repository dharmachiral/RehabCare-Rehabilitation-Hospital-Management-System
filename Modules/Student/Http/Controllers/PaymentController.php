...
{
    /**
     * Show list of payments (payments.index)
     */
    public function index()
    {
        abort_if(Gate::denies('access_payments'), 403);
    ...
    {
        abort_if(Gate::denies('create_payments'), 403);

        $students = Student::with('classModel')->get();
        
        return view('student::payments.global-form', compact('students'));
    }

    /**
     * Show payment form for selected student in global context
     */
    public function showStudentPaymentForm(Request $request)
    {
        abort_if(Gate::denies('create_payments'), 403);
        ...
        //  FETCH EXPENSES
        // ==============================
        $expenses = Expense::where('student_id', $student_id)
            ->whereIn('status', ['unpaid', 'partial'])
            ->with('expenseType')
            ->get()
        ...
        $students = Student::with('classModel')->get(); // For student dropdown

        // ==============================
        //  RETURN TO VIEW
        // ==============================
        return view('student::payments.global-form', compact(
            'student',
            'students',
            'fees',
            'expenses',
    ...
    /**
     * Use advance balance only for global payment
     */
    public function useAdvanceOnlyGlobal($student_id)
    {
        try {
            DB::beginTransaction();
            ...
            Log::info("Advance Only Global - Student: {$student->id}, Balance: {$student->balance}, Due: {$totalDueBefore}");
            ...
            DB::rollBack();
            Log::error('Global advance payment failed: ' . $e->getMessage());
    ...
    /**
     * Create payment form for specific student
     */
    public function createForStudent($student_id)
    {
        abort_if(Gate::denies('create_single_payments'), 403);
        ...
        //  RETURN TO VIEW
        // ==============================
        return view('student::payments.form', compact(
            'student',
            'fees',
            'expenses',
            'allFeeTypes',
    ...
    {
        abort_if(Gate::denies('show_payments'), 403);
    ...
    {
        $payment = Payment::with(['student', 'paymentItems.studentFee.feeType', 'paymentItems.expense.expenseType'])
            ->findOrFail($id);
        
        $totalPaid = $payment->paymentItems->sum('paid_amount');
        
        return view('student::payments.invoice', compact('payment', 'totalPaid'));
    }

    /**
     * Fix all existing monthly fees with pro-rata amounts (run this once)
     */
    public function fixAllMonthlyFees()
    {
        ...
        foreach ($students as $student) {
            $monthlyFeeType = FeeType::where('name', 'LIKE', '%monthly%')->first();
            if ($monthlyFeeType) {
                $monthlyFees = StudentFee::where('student_id', $student->id)
                    ->where('fee_type_id', $monthlyFeeType->id)
                    ->get();
                    
                foreach ($monthlyFees as $fee) {
                    $proRataData = $this->calculateProRataMonthlyFee($student, $fee);
                    
                    // Only update if the amount is different
                    if ($fee->amount != $proRataData['amount']) {
                        $fee->update([
                            'amount' => $proRataData['amount'],
                            'pro_rata_data' => json_encode($proRataData)
                        ]);
                        ...
                        $updatedCount++;
                        Log::info("Updated fee {$fee->id} from {$fee->amount} to {$proRataData['amount']} for student {$student->id}");
                    }
                }
            }
        }
        
        return "Updated {$updatedCount} monthly fees with pro-rata amounts";
    }
}