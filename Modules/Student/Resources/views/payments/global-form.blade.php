    ...
    <div class="content-wrapper">
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>Global Payment</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('payments.index') }}">Payments</a></li>
                            <li class="breadcrumb-item active">Global Payment</li>
                        </ol>
                    </div>
                </div>
            </div>
            ...
            <div class="card card-primary">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div class="d-flex flex-wrap gap-2">
                        <i class="fas fa-globe text-primary mr-2"></i> Global Payment

                    </div>

                    <div class="ml-auto">
                        <h3 class="card-title mb-0 d-flex align-items-center">
                            <a href="{{ route('payments.create') }}" class="btn btn-outline-danger shadow-sm">
                                <i class="fas fa-sync-alt mr-2"></i> Change Student
                            </a>
                ...
                @else
                    ...
                    <form action="{{ route('payments.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                            ...
                            @if ($student->balance > 0 && $totalDueAmount > 0)
                                <div class="alert alert-success">
                                    ...
                                    <div class="text-center">
                                        <a href="{{ route('payments.global.use.advance.only', $student->id) }}"
                                        ...
                                        <small class="d-block text-muted mt-1">
                                            This will use Rs.
                                            {{ number_format(min($student->balance, $totalDueAmount), 2) }}
                                    ...
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tfoot>
                                            <tr class="bg-light">
                                                ...
                                                <td class="text-right text-success">
                                                    <strong>Rs.
                                                        {{ number_format($expenses->sum('paid_amount'), 2) }}</strong>
                                                </td>
                                                <td class="text-right text-danger">
                                    ...
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($fees->where('fee_type_id', $monthlyFeeType?->id) as $i => $monthlyFee)
                                                @php
                                                    ...
                                                    $remainingAmount = max(
                                                        0,
                                                        $proRataAmount - $monthlyFee->paid_amount,
                                                    );
                                                @endphp
                                                <tr>
                                                    <td>{{ $fees->where('fee_type_id', $admissionFeeType?->id)->count() + $loop->iteration }}
                                                    </td>
                                                    <td>{{ $monthlyFee->feeType->name }}</td>
                                                    <td>{{ date('F Y', mktime(0, 0, 0, $monthlyFee->month, 1, $monthlyFee->year)) }}
                                                    </td>
                                                    <td>
                                                        @if ($proRataData['is_pro_rata'])
                                                            <small class="text-info">
                                                                ...
                                                                ({{ $proRataData['days_stayed'] }}/{{ $proRataData['total_days'] }}
                                                                days)
                                                            </small>
                                                        @else
                                            ...
                                            @if ($fees->where('fee_type_id', $monthlyFeeType?->id)->count() > 0)
                                                @php
                                                    ...
                                                    $totalMonthlyPaidAmount = $fees
                                                        ->where('fee_type_id', $monthlyFeeType?->id)
                                                        ->sum('paid_amount');
                                                    $totalMonthlyRemainingAmount = 0;

                                                    foreach (
                                                        $fees->where('fee_type_id', $monthlyFeeType?->id)
                                                        as $monthlyFee
                                                    ) {
                                                        $proRataData = $monthlyFeeDetails[$monthlyFee->id] ?? [
                                                            'amount' => $student->monthly_fee,
                                                        ...
                                                        $totalMonthlyRemainingAmount += max(
                                                            0,
                                                            $proRataAmount - $monthlyFee->paid_amount,
                                                        );
                                                    }
                                                    ...
                                                    $totalWithoutProRata =
                                                        $fees->where('fee_type_id', $monthlyFeeType?->id)->count() *
                                                        $student->monthly_fee;
                                                    $proRataAdjustment =
                                                        $totalWithoutProRata - $totalMonthlyProRataAmount;
                                                @endphp
                                        ...
                                        <tfoot>
                                            <tr class="bg-light">
                                                ...
                                                <td class="text-right text-danger">
                                                    <strong>Rs.
                                                        {{ number_format($totalMonthlyRemainingAmount, 2) }}</strong>
                                                </td>
                                                <td></td>
                                            </tr>
                                            ...
                                            @if ($proRataAdjustment > 0)
                                                <tr class="bg-light">
                                                    <td colspan="5" class="text-right text-info">
                                                        <small><i class="fas fa-info-circle"></i> Pro-rata
                                                            Adjustment:</small>
                                                    </td>
                            ...
                            @foreach ($fees->whereNotIn('fee_type_id', [$admissionFeeType?->id, $monthlyFeeType?->id]) as $i => $otherFee)
                                @php
                                    $remainingAmount =
                                        $otherFee->remaining_amount ?? $otherFee->amount - $otherFee->paid_amount;
                                @endphp
                                <tr>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" step="0.01" name="total_amount" class="form-control"
                            placeholder="Enter payment amount" required min="0.01" value="{{ old('total_amount') }}"
                            id="payment_amount">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updatePaymentPreview(enteredAmount) {
                ...
                const newAdvance = enteredAmount - amountForDues;
                ...
                if (newAdvance > 0) {
                    breakdown +=
                        `<p class="text-success"><strong>New Advance:</strong><br>Rs. ${newAdvance.toFixed(2)}</p>`;
                }
                breakdown += `</div></div>`;
                ...
                if (creditToUse > 0) {
                    breakdown +=
                        `<p class="mb-1"><i class="fas fa-credit-card"></i> <strong>Rs. ${creditToUse.toFixed(2)}</strong> will be used from your advance balance</p>`;
                }
                if (amountForDues > 0) {
                    breakdown +=
                        `<p class="mb-1"><i class="fas fa-money-bill-wave"></i> <strong>Rs. ${amountForDues.toFixed(2)}</strong> from new payment will cover remaining dues</p>`;
                }
                if (newAdvance > 0) {
                    breakdown +=
                        `<p class="mb-1 text-success"><i class="fas fa-piggy-bank"></i> <strong>Rs. ${newAdvance.toFixed(2)}</strong> will be added to your advance balance</p>`;
                }
                breakdown +=
                    `<p class="mb-0"><i class="fas fa-balance-scale"></i> <strong>Final Advance Balance:</strong> Rs. ${remainingCredit.toFixed(2)}</p>`;
                breakdown += `</div>`;
                ...
                if (totalCovered < totalDueAmount) {
                    ...
                    breakdown +=
                        `<p class="text-warning mb-0"><i class="fas fa-exclamation-triangle"></i> <strong>Partial Payment:</strong> This will not cover all due amounts. Remaining due: Rs. ${(totalDueAmount - totalCovered).toFixed(2)}</p>`;
                    breakdown += `</div>`;
                }
                ...
                if (totalCovered >= totalDueAmount) {
                    ...
                    breakdown +=
                        `<p class="text-success mb-0"><i class="fas fa-check-circle"></i> This payment will fully cover all due amounts</p>`;
                    breakdown += `</div>`;
                }
